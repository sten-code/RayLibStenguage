...
namespace RayLibStenguage
{
    public class RText
    {
            ...
            ListValue chars, ListValue recs, NumberValue glyphCount, NumberValue fontSize, NumberValue padding, NumberValue packMethod)
        {
            ...
            return new RuntimeResult().Success(new Image(Imports.GenImageFontAtlas(
                chars.Items
                    .Where(x => x.GetType() == typeof(GlyphInfo))
                    .Select(x => ((GlyphInfo)x).Struct)
                    .ToArray(),
                (Structs.Rectangle[]*)recsHandle.AddrOfPinnedObject().ToPointer(),
                (int)glyphCount.Value,
                (int)fontSize.Value,
                (int)padding.Value,
        ...
        {
            return new RuntimeResult().Success(new NumberValue(Imports.LoadCodepoints(text.Value, (int)count.Value)));
        }
